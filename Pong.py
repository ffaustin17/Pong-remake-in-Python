...
wn.onkeypress(exit_var.set_exit_true, "q")

#Main game loop
while True:
    wn.update()
    #exit the game if the user wants to
    if(exit_var.exit_value == True):
        break

    #move the ball
    new_x_coord = ball.xcor() + ball.dx
    ball.setx(new_x_coord)

    new_y_coord = ball.ycor() + ball.dy
    ball.sety(new_y_coord)
